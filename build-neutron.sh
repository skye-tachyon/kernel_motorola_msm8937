...
if ! [ -d "$TC_DIR" ]; then
    ...
    mkdir -p "$TC_DIR"
    if ! wget -O "$HOME/toolchains/neutron-clang.tar.zst" https://github.com/Neutron-Toolchains/clang-build-catalogue/releases/download/10032024/neutron-clang-10032024.tar.zst; then
        echo "Cloning failed! Aborting..."
        exit 1
    fi
    tar -I zstd -xf $HOME/toolchains/neutron-clang.tar.zst -C "$TC_DIR"
    rm -rf $HOME/toolchains/neutron-clang.tar.zst
fi
...
if [ -f "$kernel" ]; then
    ...
    if [ "$NOZIP" = false ]; then
        ...
        elif ! git clone -q https://github.com/skye-tachyon/AnyKernel3- -b $DEVICE AnyKernel3; then
