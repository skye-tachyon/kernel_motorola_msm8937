...
#ifndef MAX_PANEL_SUPPLIERS
#define MAX_PANEL_SUPPLIERS 5
#endif

/* Stub out vendor arrays in platform_data for 4.9 (fields removed) */
struct ft5x06_ts_platform_data;

/* FPS notifier is Android-vendor specific in 3.18; make it a no-op on 4.9 */
#ifndef FPS_register_notifier
#define FPS_register_notifier(...) 0
#endif
...
{
	/* Vendor mapping removed on 4.9; return NULL */
	return NULL;
}
			...
			struct ft5x06_ts_platform_data *pdata)
{
...
#if 0 /* vendor mapping removed in 4.9 */
	...
	if ((num_vendor_ids > 0) && (num_vendor_ids < MAX_PANEL_SUPPLIERS)) {
		...
		rc = of_property_read_u32_array(np,
			"focaltech,vendor_ids", (NULL /*vendor_ids removed in 4.9*/ ),
			num_vendor_ids);
		if (rc) {
		...
		for (name = of_prop_next_string(prop, NULL), i = 0;
			...
			(NULL /*vendor_names removed in 4.9*/ )[i] = name;

		0 /*num_vendor_ids removed in 4.9*/ = num_vendor_ids;
	}
#endif
