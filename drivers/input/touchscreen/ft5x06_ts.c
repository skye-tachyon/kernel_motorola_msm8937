...
#endif
	/* --- MOTO CHANGES START: Struct Members --- */
...
/* --- MOTO CHANGES START: Logic Functions --- */
static int ft5x06_set_charger_state(struct ft5x06_ts_data *data, bool enable) {
	return ft5x0x_write_reg(data->client, 0x8B, enable ? 1 : 0);
}

static void ft5x06_update_ps_chg_cm_state(struct ft5x06_ts_data *data, bool present) {
...
static int ps_get_state(struct ft5x06_ts_data *data, struct power_supply *psy, bool *present) {
	union power_supply_propval pval = {0};
	int retval = power_supply_get_property(psy, POWER_SUPPLY_PROP_PRESENT, &pval);
...
static void ps_notify_callback_work(struct work_struct *work) {
	struct ft5x06_ts_data *ft5x06_data = container_of(work, struct ft5x06_ts_data, ps_notify_work);
	bool present = ft5x06_data->ps_is_present;
	ft5x06_set_charger_state(ft5x06_data, present);
	ft5x06_update_ps_chg_cm_state(ft5x06_data, present);
}

static int ps_notify_callback(struct notifier_block *self, unsigned long event, void *data) {
	struct ft5x06_ts_data *ft5x06_data = container_of(self, struct ft5x06_ts_data, ps_notif);
	struct power_supply *psy = data;
	bool present;
	if ((event == PSY_EVENT_PROP_ADDED || event == PSY_EVENT_PROP_CHANGED) &&
		psy && !strncmp(psy->desc->name, "usb", 3)) {
		if (ps_get_state(ft5x06_data, psy, &present) == 0) {
...
static struct config_modifier *modifier_by_id(struct ft5x06_ts_data *data, int id) {
...
/* Mock FPS register se nao existir no kernel base */
#ifndef FPS_register_notifier
int __attribute__((weak)) FPS_register_notifier(struct notifier_block *nb, unsigned long action, bool atomic) { return 0; }
#endif

static int fps_notifier_callback(struct notifier_block *self, unsigned long event, void *data) {
	int fps_state = *(int *)data;
	struct ft5x06_ts_data *ft5x06_data = container_of(self, struct ft5x06_ts_data, fps_notif);
	if (ft5x06_data && event == 0xBEEF) {
		struct config_modifier *cm = modifier_by_id(ft5x06_data, FT_MOD_FPS);
		if (cm) {
			if (fps_state) {
...
static void ft5x06_detection_work(struct work_struct *work) {
	struct ft5x06_ts_data *data = exp_fn_ctrl.ft5x06_ts_data_ptr;
	if (data && data->fps_detection_enabled && !data->is_fps_registered) {
		if (FPS_register_notifier(&data->fps_notif, 0xBEEF, false) == 0)
			data->is_fps_registered = true;
		else
			queue_delayed_work(exp_fn_ctrl.det_workqueue, &exp_fn_ctrl.det_work, msecs_to_jiffies(EXP_FN_DET_INTERVAL));
	}
}

static int ft5x06_modifier_name2id(const char *name) {
	if (!strncmp(name, "charger", 7)) return FT_MOD_CHARGER;
	if (!strncmp(name, "fps", 3)) return FT_MOD_FPS;
	return -1;
}

static void ft5x06_dt_parse_modifier(struct ft5x06_ts_data *data, struct device_node *parent, struct config_modifier *config, const char *modifier_name, bool active) {
	char node_name[64];
	struct ft5x06_clip_area clipa;
	struct device_node *np_config;
	scnprintf(node_name, 63, "%s-%s", modifier_name, active ? "active" : "suspended");
	np_config = of_find_node_by_name(parent, node_name);
	if (np_config) {
		if (!of_property_read_u32_array(np_config, "touch-clip-area", (unsigned int *)&clipa, sizeof(clipa)/sizeof(unsigned int))) {
			config->clipa = devm_kzalloc(&data->client->dev, sizeof(clipa), GFP_KERNEL);
			if (config->clipa) memcpy(config->clipa, &clipa, sizeof(clipa));
		}
...
static int ft5x06_dt_parse_modifiers(struct ft5x06_ts_data *data) {
	...
	for (i = 0; i < num_names; i++) {
		if (of_property_read_string_index(np, "config_modifier-names", i, &mod_name) == 0) {
			struct config_modifier *config = devm_kzalloc(&data->client->dev, sizeof(*config), GFP_KERNEL);
			config->name = mod_name;
			config->id = ft5x06_modifier_name2id(mod_name);
			list_add_tail(&config->link, &data->modifiers.mod_head);
			
			char node_name[64];
			scnprintf(node_name, 63, "config_modifier-%s", mod_name);
			struct device_node *np_mod = of_find_node_by_name(np, node_name);
			if (np_mod) {
				if (of_property_read_bool(np_mod, "enable-notification")) {
					if (config->id == FT_MOD_CHARGER) data->charger_detection_enabled = true;
					if (config->id == FT_MOD_FPS) data->fps_detection_enabled = true;
				} else {
					config->effective = true;
				}
				ft5x06_dt_parse_modifier(data, np_mod, config, mod_name, true);
				of_node_put(np_mod);
			}
		}
	}
...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		/* --- MOTO CHANGES START: Clipping --- */
		if (data->clipping_on && data->clipa) {
			bool inside = (x >= data->clipa->xul_clip) && (x <= data->clipa->xbr_clip) &&
						  (y >= data->clipa->yul_clip) && (y <= data->clipa->ybr_clip);
			   ...
			   const struct i2c_device_id *id)
{
	...
	/* --- MOTO CHANGES START: Init --- */
	data->patching_enabled = true;
	ft5x06_dt_parse_modifiers(data);
	...
	if (data->charger_detection_enabled) {
		...
		if (!power_supply_reg_notifier(&data->ps_notif)) {
			psy = power_supply_get_by_name("usb");
			if (psy) {
				ps_get_state(data, psy, &data->ps_is_present);
				ft5x06_set_charger_state(data, data->ps_is_present);
				ft5x06_update_ps_chg_cm_state(data, data->ps_is_present);
	...
	if (data->fps_detection_enabled) {
		...
		exp_fn_ctrl.det_workqueue = create_singlethread_workqueue("ft5x06_det_workqueue");
		INIT_DELAYED_WORK(&exp_fn_ctrl.det_work, ft5x06_detection_work);
		queue_delayed_work(exp_fn_ctrl.det_workqueue, &exp_fn_ctrl.det_work, msecs_to_jiffies(EXP_FN_DET_INTERVAL));
	}
